...
<%
	DtEdicion dte = (DtEdicion) request.getAttribute("edicion");
    DtUsuario dtu = (DtUsuario) request.getAttribute("organizador");
    List<String> treg = (List<String>) request.getAttribute("tiposReg");
    List<DtPatrocinio> pats = (List<DtPatrocinio>) request.getAttribute("pats");  
    List<DtCompra> compras = (List<DtCompra>) request.getAttribute("compras");
	String rutaImagen =(dte.getImagen().equals(""))?"defaultEdicion.png":dte.getImagen();
	String imgORganizador =(dtu.getImagen().equals(""))?"defaultUser.png":dtu.getImagen();
			    ...
			    <div class="col-xl-4 col-lg-5 col-md-6 col-sm-6 mx-auto">
			  	<div class="row  c5 cLetraRelleno text-center bordes mx-auto ">
			    	...
			    	<div class="row text-start negrita mt-3 ms-1" style="Font-size:20px;">
			    		<p class="mid sinSubrayado" href="SrvUsuario?destino=detalleUsuario&nick=<%=dtu.getNickname()%>">	
			    		...
			    		<p class="sinSubrayado" href="SrvUsuario?destino=detalleUsuario&nick=<%=dtu.getNickname()%>" style="text-align: center;"> <%=dtu.getNombre() %> </p>
					</div>
	       		  </div>
			    	...
			    	<div id="tiposRegistro" class="row text-start negrita mt-3 ms-1" style="Font-size:20px;">
			    	  <%
                       ...
                       for (String t : treg) {
                      %>                      
                      <p class="sinSubrayado" href="SrvTipoRegistro?destino=detalleTipoRegistro&treg=<%= t %>&edi=<%=dte.getNombre()%>&imagen=<%=rutaImagen%>" style="text-align: center;"><%= t %> </p>
                      <%
                         }
                          }
                       %>
			        ...
			        <%
			          ...
			          <div class="row  c5 cLetraRelleno text-center bordes mx-auto mt-4">
	       		     ...
	       		     %>
			           <a class="sinSubrayado" href="SrvUsuario?destino=detalleUsuario&nick=<%= comp.getAsistente() %>" style="text-align: center;"><%= comp.getAsistente() %> </a>
					 <% 
	       		    	  }
	       		        }
			           }
